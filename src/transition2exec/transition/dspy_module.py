...
class Transition2ExecChainOfThought(dspy.Module):
    ...
    def _extract_resolved_tools(self, tool_plan: str, request: Transition2ExecRequest) -> str:
        ...
        # ISS-020 guard (dual-signal): block wrong-tool fallbacks only when BOTH signals fail.
        # Signal 1 — token overlap: abstract_tool name shares a word with some catalog tool name.
        # Signal 2 — output overlap: selected tool's output keys intersect abstract required outputs.
        # Domain-specific abstract tools (e.g. duplicate_config → copy_file) have no token overlap
        # but do have output key overlap when Stage 1 is correct — so one signal passing is enough.
        abstract_tool_words = set(
            request.abstract_transition.tool.lower().replace("_", " ").split()
        )
        catalog_words = set(
            w for t in request.available_tools
            for w in t.name.lower().replace("_", " ").split()
        )
        abstract_output_set = set(request.abstract_transition.outputs)
        token_match = bool(abstract_tool_words & catalog_words)
        output_match = any(set(out.keys()) & abstract_output_set for _, _, out in selected)
        if not token_match and not output_match:
            return "(none matched)"
